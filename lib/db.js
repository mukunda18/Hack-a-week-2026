...
export const getOfficesSeverityMap = async () => {
  const res = await pool.query(
    `SELECT 
       ...
       ot.name as office_type,
       COUNT(r.id) as report_count,
       COALESCE(SUM(r.bribe_amount * r.confidence_score) / NULLIF(SUM(r.confidence_score), 0), 0) as avg_bribe,
       COALESCE(SUM(r.delay * r.confidence_score) / NULLIF(SUM(r.confidence_score), 0), 0) as avg_delay,
       COALESCE(AVG(r.confidence_score), 0) as avg_confidence,
       MAX(r.created_at) as last_report_date,
       LEAST(100, 
         (LN(COUNT(r.id) + 1) * 15) + 
         (LN(COALESCE(SUM(r.bribe_amount * r.confidence_score) / NULLIF(SUM(r.confidence_score), 0), 0) + 1) * 5) + 
         (COALESCE(SUM(r.delay * r.confidence_score) / NULLIF(SUM(r.confidence_score), 0), 0) * 2)
       ) as severity_score
     FROM offices o
     JOIN office_types ot ON o.office_type_id = ot.id
