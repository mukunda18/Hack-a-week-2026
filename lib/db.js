...
export const addReportRaw = async (report) => {
  ...
  const queryText = `
    INSERT INTO reports (
      office_id, ip_hash_id, bribe_amount, delay, report_date, 
      confidence_score, created_at, service_type, description, 
      visit_time, interaction_method, outcome
...
export const getOfficesSeverityMap = async () => {
  const res = await pool.query(
    `SELECT 
       ...
       COUNT(r.id) as report_count,
       COALESCE(
         SUM(r.bribe_amount * r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)) / 
         NULLIF(SUM(r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)), 0), 
         0
       ) as avg_bribe,
       COALESCE(
         SUM(r.delay * r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)) / 
         NULLIF(SUM(r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)), 0), 
         0
       ) as avg_delay,
       ...
       LEAST(100, 
         (LN(COALESCE(SUM(r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)), 0) + 1) * 15) + 
         (LN(COALESCE(
           SUM(r.bribe_amount * r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)) / 
           NULLIF(SUM(r.confidence_score * (EXTRACT(EPOCH FROM (r.created_at - (NOW() - INTERVAL '90 days'))) / 7776000.0)), 0), 
           0
     ...
     FROM offices o
     JOIN office_types ot ON o.office_type_id = ot.id
     LEFT JOIN reports r ON o.id = r.office_id AND r.created_at > NOW() - INTERVAL '90 days'
     GROUP BY o.id, ot.name
  ...
  return res.rows.map(office => {
    ...
    const radius = reportCount > 0
      ? Math.min(10 + (Math.sqrt(reportCount) * 8) + (normalizedScore * 15), 50)
      : 0;

    const hue = Math.max(0, 40 - (normalizedScore * 40));
    const opacity = 0.4 + (normalizedScore * 0.45);
    const color = `hsla(${hue}, 100%, 50%, ${opacity})`;
    const borderColor = `hsla(${hue}, 100%, 30%, 0.8)`;
